...
const $21_STRICT_LIMITS = 0b10

const $22_HOMING_ENABLED = 0b00000001
const $22_SINGLE_AXIS_HOMING = 0b00000010
const $22_HOME_ON_START = 0b00000100
const $22_ORIGIN_IS_ZERO = 0b00001000
const $22_GANGED_INPUTS = 0b00010000
const $22_MANUAL_HOMING = 0b00100000
const $22_OVERRIDE_HOMING_LOCK = 0b01000000
const $22_STAY_HOMED_ON_RESET = 0b10000000
...
/*function readFluidBackupFile(evt) {
...
/*function loadFluidBackupFile(f) {
    if (f) {
        // Filereader
...
function fluidSettings(data) {
  ...
  var axesWithHoming = 0;
  var axesWithSingleHoming = 3;
  var axesWithSoftLimits = 0;
  var axesWithHardLimits = 0;
  var mustHomeOnStart = true;

  var fluidToGrblConfig = {
    $20: 0,
    $21: 0,
    $22: 0,
    $27: 0,
    $30: 0,
    $31: 0,
    $32: _.has(fluidncJSON, "laser") ? 1 : 0,
    $100: _.get(fluidncJSON, "axes.x.steps_per_mm", 0),
    $101: _.get(fluidncJSON, "axes.y.steps_per_mm", 0),
    $102: _.get(fluidncJSON, "axes.z.steps_per_mm", 0),
    $103: _.get(fluidncJSON, "axes.a.steps_per_mm", 0),
    $110: _.get(fluidncJSON, "axes.x.max_rate_mm_per_min", 0),
    $111: _.get(fluidncJSON, "axes.y.max_rate_mm_per_min", 0),
    $112: _.get(fluidncJSON, "axes.z.max_rate_mm_per_min", 0),
    $113: _.get(fluidncJSON, "axes.a.max_rate_mm_per_min", 0),
    $120: _.get(fluidncJSON, "axes.x.acceleration_mm_per_sec2", 0),
    $121: _.get(fluidncJSON, "axes.y.acceleration_mm_per_sec2", 0),
    $122: _.get(fluidncJSON, "axes.z.acceleration_mm_per_sec2", 0),
    $123: _.get(fluidncJSON, "axes.a.acceleration_mm_per_sec2", 0),
    $130: _.get(fluidncJSON, "axes.x.max_travel_mm", 0),
    $131: _.get(fluidncJSON, "axes.y.max_travel_mm", 0),
    $132: _.get(fluidncJSON, "axes.z.max_travel_mm", 0),
    $133: _.get(fluidncJSON, "axes.a.max_travel_mm", 0)
  }

  axesWithHardLimits += _.get(fluidncJSON, "axes.x.motor0.hard_limits", 0);
  axesWithHardLimits += _.get(fluidncJSON, "axes.y.motor0.hard_limits", 0);
  axesWithHardLimits += _.get(fluidncJSON, "axes.z.motor0.hard_limits", 0);
  axesWithHardLimits += _.get(fluidncJSON, "axes.a.motor0.hard_limits", 0);

  axesWithSoftLimits += _.get(fluidncJSON, "axes.x.soft_limits", 0);
  axesWithSoftLimits += _.get(fluidncJSON, "axes.y.soft_limits", 0);
  axesWithSoftLimits += _.get(fluidncJSON, "axes.z.soft_limits", 0);
  axesWithSoftLimits += _.get(fluidncJSON, "axes.a.soft_limits", 0);

  axesWithHoming += _.has(fluidncJSON, "axes.x.homing");
  axesWithHoming += _.has(fluidncJSON, "axes.y.homing");
  axesWithHoming += _.has(fluidncJSON, "axes.z.homing");
  axesWithHoming += _.has(fluidncJSON, "axes.a.homing");

  axesWithSingleHoming += _.get(fluidncJSON, "axes.x.homing.allow_single_axis", 1);
  axesWithSingleHoming += _.get(fluidncJSON, "axes.y.homing.allow_single_axis", 1);
  axesWithSingleHoming += _.get(fluidncJSON, "axes.z.homing.allow_single_axis", 1);
  axesWithSingleHoming += _.get(fluidncJSON, "axes.a.homing.allow_single_axis", 1);

  mustHomeOnStart = _.get(fluidncJSON, "start.must_home", true);

  const maxSpeed = _.head(_.invoke(_.get(_.find(fluidncJSON, "speed_map"), "speed_map"), "match", /(\d+)(?==100.?\d*%)/));
  const minSpeed = _.head(_.invoke(_.get(_.find(fluidncJSON, "speed_map"), "speed_map"), "match", /(\d+)(?==0.?\d*%)/));

  fluidToGrblConfig.$20 = (axesWithSoftLimits > 0) ? 1 : 0;
  fluidToGrblConfig.$21 = (axesWithHardLimits > 0) ? 1 : 0;
  fluidToGrblConfig.$22 |= (axesWithHoming > 0) ? $22_HOMING_ENABLED : 0;
  if (fluidToGrblConfig.$22 > 0) {
    fluidToGrblConfig.$22 |= (axesWithSingleHoming > 0) ? $22_SINGLE_AXIS_HOMING : 0;
    fluidToGrblConfig.$22 |= mustHomeOnStart ? $22_HOME_ON_START : 0;
  }
  if (maxSpeed != null) {
    fluidToGrblConfig.$30 = maxSpeed;
  }
  if (minSpeed != null) {
    fluidToGrblConfig.$31 = minSpeed;
  }

  grblParams = fluidToGrblConfig;
